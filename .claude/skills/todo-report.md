Generate a status report for the todo system. Use `templates/report.md` as the structural reference.
...
2. **Queue (Inbox)**: List all `.md` files in `inbox/` (excluding `.gitkeep`), sorted lexicographically ascending. For each, extract `title` and `created` from frontmatter. Compute age as (today - created date).
